...
describe("floorToBucket", () => {
  // ── UTC modular floor at every bucket size ───────────────────────
  // The whole app's timestamp display renders in UTC for parity with
  // the native Davis Problems app (see TIMEZONE CONVENTION docblock
  // in utils/formatters.ts). Chart bucket alignment matches: pure
  // modular floor on the UTC ms timeline, no local-TZ shift.
  // (Earlier 0.0.76 attempted LOCAL alignment for day-and-larger
  //  buckets so labels matched the user's calendar, but with the
  //  display flipped to UTC everywhere in 0.0.86 the local floor
  //  re-introduces the same kind of mismatch in the opposite
  //  direction.)
  ...
  it("aligns a DAY bucket to UTC midnight of that timestamp's calendar day", () => {
    // 2026-05-18T14:30:00Z (Sun) → floored to 2026-05-18T00:00:00Z.
    const t = Date.UTC(2026, 4, 18, 14, 30, 0, 0);
    const expected = Date.UTC(2026, 4, 18, 0, 0, 0, 0);
    expect(floorToBucket(t, DAY)).toBe(expected);
  });

  it("DAY bucket: timestamps just after UTC midnight stay on the same day", () => {
    const t = Date.UTC(2026, 4, 18, 0, 1, 0, 0);
    const expected = Date.UTC(2026, 4, 18, 0, 0, 0, 0);
    expect(floorToBucket(t, DAY)).toBe(expected);
  });

  it("DAY bucket: timestamps just before UTC midnight stay on the previous day", () => {
    const t = Date.UTC(2026, 4, 17, 23, 59, 0, 0);
    const expected = Date.UTC(2026, 4, 17, 0, 0, 0, 0);
    expect(floorToBucket(t, DAY)).toBe(expected);
  });

  it("multi-DAY (3d) bucket: floor is invariant within the bucket span", () => {
    const t = Date.UTC(2026, 4, 18, 14, 0);
