 ...
 *
 *  TIMEZONE: pure modular floor on the UTC ms timeline at every
 *  bucket size. The whole app's timestamp display now renders in
 *  UTC for parity with the native Davis Problems app (see TIMEZONE
 *  CONVENTION docblock in utils/formatters.ts), so the bucket
 *  boundaries land on the same UTC anchors the labels show.
 *
 *  History: in 0.0.76 this floored DAY-and-larger buckets to LOCAL
 *  midnight (to fix "May 18" bars showing late-May-17 data for
 *  Brazil users when the labels were local). With the display
 *  switched to UTC everywhere (0.0.86), the local alignment would
 *  create the opposite mismatch — bars whose START is local
 *  midnight but whose LABEL says "UTC midnight". UTC floor +
 *  UTC labels = consistent everywhere. */
